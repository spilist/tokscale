...
pub fn parse_codex_file(path: &Path) -> Vec<UnifiedMessage> {
    ...
    for line in reader.lines() {
        ...
        let model = current_model.clone().unwrap_or_else(|| "unknown".to_string());
        ...
        let (input, output, cached) = if let Some(last) = &info.last_token_usage {
            (
                ...
                last.cached_input_tokens.or(last.cache_read_input_tokens).unwrap_or(0),
            )
            ...
            let curr_cached = total.cached_input_tokens.or(total.cache_read_input_tokens).unwrap_or(0);
        ...
        if let Some(total) = &info.total_token_usage {
            previous_totals = Some((
                ...
                total.cached_input_tokens.or(total.cache_read_input_tokens).unwrap_or(0),
            ));
        }
